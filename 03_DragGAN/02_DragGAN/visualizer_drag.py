...
class Visualizer(imgui_window.ImguiWindow):
    ...
    def draw_frame(self):
        is_detected = self.drag_widget.is_detected
        self.begin_frame()
        self.args = dnnlib.EasyDict()
        ...
        if 'image' in self.result:
            ...
            if not is_detected:
                ...
                for point in self.drag_widget.targets:
                    pos_x = self.pane_w + max_w / 2 + (point[0] - self.image_w//2) * zoom
                    pos_y = max_h / 2 + (point[1] - self.image_h//2) * zoom
                    gl_utils.draw_circle(center=np.array([pos_x, pos_y]), color=[0,0,1], radius=4 * rescale)
                
                for point in self.drag_widget.points:
                    pos_x = self.pane_w + max_w / 2 + (point[0] - self.image_w//2) * zoom
                    pos_y = max_h / 2 + (point[1] - self.image_h//2) * zoom
                    gl_utils.draw_circle(center=np.array([pos_x, pos_y]), color=[1,0,0], radius=4 * rescale)

                for point, target in zip(self.drag_widget.points, self.drag_widget.targets):
                    t_x = self.pane_w + max_w / 2 + (target[0] - self.image_w//2) * zoom
                    t_y = max_h / 2 + (target[1] - self.image_h//2) * zoom

                    p_x = self.pane_w + max_w / 2 + (point[0] - self.image_w//2) * zoom
                    p_y = max_h / 2 + (point[1] - self.image_h//2) * zoom

                    gl_utils.draw_arrow(p_x, p_y, t_x, t_y, l=4 * rescale, width = 1.5 * rescale)

            # for point in self.drag_widget.targets:
            #     gl_utils.draw_circle(center=np.array(point), color=[0,0,1], radius=9 * rescale)

            # for point in self.drag_widget.points:
            #     gl_utils.draw_circle(center=np.array(point), color=[1,0,0], radius=9 * rescale)

            # for point, target in zip(self.drag_widget.points, self.drag_widget.targets):
            #     gl_utils.draw_arrow(point[0], point[1], target[0], target[1], l=8 * rescale, width = 3 * rescale)
