...
class Renderer:
    ...
    def _render_drag_impl(self, res,
        ...
        **kwargs
        ...
        if img_normalize:
            norm_factor = img.norm(float('inf'), dim=[1,2], keepdim=True).clip(1e-8, 1e8)
            img = img / norm_factor
        ...
        # img_np = np.ascontiguousarray(img_raw.detach().cpu().numpy().transpose(1, 2, 0).astype(np.uint8))

        img_np = np.ascontiguousarray(img_np)
       

        # 确保 dtype 为 uint8（OpenCV 需要这种格式）
        img_np = img_np.astype(np.uint8)


        self.landmarks = self.fa.get_landmarks(img_np)



        # # 如果是灰度图像（2D），转换为 RGB（3D）
        # if img_np.ndim == 2:
        #     img_np = cv2.cvtColor(img_np, cv2.COLOR_GRAY2BGR)

        # 确保数组是可写的（从张量转换的数组有时是只读的）
        # img_np.setflags(write=True)


        # self.points.append(landmarks[0].tolist())

        # print(f"img_np shape: {img_np.shape}, dtype: {img_np.dtype}")
        # print(f"img_np type: {type(img_np)}")

        # # 使用 face-alignment 检测人脸关键点

        # print(f"landmarks : {landmarks}")
        # print(f"landmarks[0] : {landmarks[0]}")
        # print(f"landmarks[0][0] shape : {landmarks[0][0].shape}")
        ...
        #         x, y = face  # 提取坐标
        #         img_np = cv2.circle(img_np, (int(x), int(y)), radius=2, color=(255, 0, 0), thickness=-1)

        # fig, ax = plt.subplots(1, 1, figsize=(8, 8))
        # ax.imshow(img_np)
        # plt.show()

        # # 如果需要将修改后的 NumPy 图像转换回 PyTorch 张量
